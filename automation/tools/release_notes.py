#!/usr/bin/env python3
"""Helpers for /release-notes v4 — parse batches, build JQL, format markdown."""
...
def _scope_format_vars() -> dict[str, str]:
    ...
    return {
        ...
        "pmoproc_retail_banking": ", ".join(s["pmoproc_retail_banking"]),
        ...
        "status_exclude_retail_banking_base": _jql_list(
            s["status_exclude_retail_banking_base"]
        ),
...
def _lane_suffix(lane: str) -> str:
    c = load_contract()["output"]
    if lane == "fx":
        return c["fx_suffix"]
    if lane == "rb":
        return c["rb_suffix"]
    return c["non_fx_suffix"]
...
def merge_version_data(
    ...
    rb_versions: dict[str, dict[str, list]],
    master_adaptive: dict[str, list[dict]],
    fx_adaptive: dict[str, list[dict]],
    rb_adaptive: dict[str, list[dict]],
    version: str,
    *,
    ...
    _add_issues_to_sections(rb_versions, version, rb_base, set())
    ...
    for raw in master_base:
        ...
        rb_children = _can_to_children(rb_adapt_can)
        if rb_children:
            rb_adaptive.setdefault(version, []).append({**base_block, "children": rb_children})


def assemble_batch_markdown(batch: dict, version_payloads: list[dict]) -> tuple[str, str, str]:
    """version_payloads: [{version, master_base[], fx_base[], rb_base[], master_adaptive[], fx_adaptive[], rb_adaptive[]}]"""
    master_v: dict[str, dict[str, list]] = {}
    fx_v: dict[str, dict[str, list]] = {}
    rb_v: dict[str, dict[str, list]] = {}
    master_a: dict[str, list[dict]] = {}
    fx_a: dict[str, list[dict]] = {}
    rb_a: dict[str, list[dict]] = {}
...
def cmd_assemble_from_raw(args: argparse.Namespace) -> None:
    ...
    master_md, fx_md, rb_md = assemble_batch_markdown(batch, payloads)
    out = {
    ...
    print(
        json.dumps(
            {"master_len": len(master_md), "fx_len": len(fx_md), "rb_len": len(rb_md)},
            indent=2,
        )
    )
...
_QUERY_IDS = [
    ...
    "fx_spot_adaptive",
    "master_adaptive",
    "retail_banking_adaptive",
]


def main() -> None:
    p = argparse.ArgumentParser(description="Release notes helpers (v4)")
    ...
    bj.add_argument("--query", required=True, choices=_QUERY_IDS)
    ...
    wb.add_argument("--lane", choices=["fx", "non_fx", "rb"], required=True)
