#!/usr/bin/env python3
"""
PostgreSQL over SSH — PuTTY plink-first on Windows, OpenSSH fallback.

Typical flow (two Cursor terminal tabs):

  Tab 1 — tunnel (interactive SSH password when plink prompts):
    python automation/tools/tunnel/ctqa_pg.py USER@jump.example.com
  ...
  Cursor MCP: add postgres-readonly to global Cursor mcp.json (~/.cursor/mcp.json;
  on Windows, under %USERPROFILE%/.cursor/mcp.json). Template: repo file
  .cursor/mcp/postgres-readonly.mcp.json. URI: 127.0.0.1:15432, sslmode=disable
...
def parse_jump(jump: str) -> tuple[str, str]:
    if "@" not in jump:
        raise argparse.ArgumentTypeError(
            "JUMP must be user@host (e.g. you@jump.example.com)"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SSH tunnel to Postgres (plink on Windows by default) + optional DB probe.",
    ...
    parser.add_argument(
        ...
        nargs="?",
        metavar="USER@HOST",
        help="SSH target (required for tunnel mode), e.g. you@jump.example.com",
    )
    parser.add_argument(
        "--probe-only",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "Use your DB hostname if the server reaches Postgres only via a specific FQDN.",
    )
    parser.add_argument(
        "--remote-db-port",
        type=int,
