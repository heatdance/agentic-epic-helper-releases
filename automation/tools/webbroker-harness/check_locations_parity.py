#!/usr/bin/env python3
"""Parity check: compass features in WebBroker concept-map vs keys in locations/ctqa.json."""
...
def main() -> int:
    ...
    args = p.parse_args()
    root: Path = args.repo
    concept = root / "docs" / "webbroker-harness" / "concept-map.json"
    ctqa = root / "docs" / "webbroker-harness" / "locations" / "ctqa.json"

    if not concept.is_file() or not ctqa.is_file():
        print("Missing docs/webbroker-harness/concept-map.json or locations/ctqa.json", file=sys.stderr)
        return 2
