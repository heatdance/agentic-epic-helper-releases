...
from epic_stats.categories import classify_epic, load_categories, tcd_size_band
...
CONTRACT_PATH = REPO_ROOT / "docs" / "epic-stats-contract.json"
SCHEMA_VERSION = 5
...
def _stats_root(contract: dict[str, Any]) -> Path:
    rel = (contract.get("paths") or {}).get("stats_root", "stats/epic-stats")
...
def state_path_for_user(jira_user: str, contract: dict[str, Any] | None = None) -> Path:
    ...
    tmpl = (c.get("paths") or {}).get(
        "state_template", "stats/epic-stats/state/last-sync-{jira_user}.json"
    )
...
def latest_path_for_user(jira_user: str, contract: dict[str, Any] | None = None) -> Path:
    c = contract or load_contract()
    tmpl = (c.get("paths") or {}).get(
        "latest_template", "stats/epic-stats/latest-{jira_user}.md"
    )
    return REPO_ROOT / tmpl.format(jira_user=jira_user)
...
def latest_team_path(contract: dict[str, Any] | None = None) -> Path:
    c = contract or load_contract()
    rel = (c.get("paths") or {}).get("latest_team", "stats/epic-stats/latest-team.md")
    return REPO_ROOT / rel
...
def raw_run_path(jira_user: str, contract: dict[str, Any] | None = None) -> Path:
    c = contract or load_contract()
    tmpl = (c.get("paths") or {}).get(
        "raw_template", "stats/epic-stats/raw/{jira_user}/run-{utc}.jsonl"
    )
...
def incremental_missing_state_message(jira_user: str) -> str:
    return (
        f"No state file for `{jira_user}`. Run `/epic-stats` with "
