...
from epic_stats.classify_epic_from_text import suggest_category  # noqa: E402
from epic_stats.apply_categories import epic_categories_path, load_epic_categories, save_epic_categories  # noqa: E402
...
def merge_meta() -> dict[str, dict]:
    ...
    for user in USERS:
        path = REPO / "stats/epic-stats/state" / f"last-sync-{user}.json"
        if not path.is_file():
            continue
