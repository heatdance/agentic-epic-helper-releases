...
| Master | `master_base` | `master_adaptive` |
...
**Adaptive:** detect `update adaptive` on master base scan; pull token after `to` → `corner-adaptive-{token}`; run both adaptive JQLs (no `project=`); nest CAN children under the trigger in the correct file.
