...
- **Transcribe the whole parameter table** into `snippet_text` — every row, not the rows you plan to check. `snippet_text` shorter than **60%** of the fetched source section fails finalize (`snippet_truncated`), because a thin snippet silently lowers every downstream breadth threshold.
- After a successful extract: fill **`parameter_inventory[]`** (`name`, `spec_text`, `source_line`) from the snippet structure (markdown table rows or `Name — Spec` / `Name: Spec` lines) — do **not** invent names absent from the snippet. Cover **every** row the snippet shows: `epic_prep_verify` re-derives the inventory from `snippet_text` and fails on omissions (`parameter_inventory_undercut`), so a declared inventory can only add to the derived one, never narrow it. Apply [`docs/variation-catalogue.json`](../../docs/variation-catalogue.json); list unmatched cells in **`unmatched_spec_patterns[]`**.
...
7. **Record the pass (mandatory).** After each requirement subprocess, append one **`requirement_passes[]`** entry to the ref: `{ "requirement_key", "subprocess": true, "snippet_chars", "source_chars", "inventory_rows", "mandated_variations", "completed_at" }`. `snippet_chars` and `inventory_rows` are **recomputed** by `epic_prep_verify`, and every requirement with `snippet_status: ok` needs an entry — a one-shot sweep that skips requirements fails finalize instead of shipping thin coverage.
8. When `snippet_status` is **failed**: emit **one** `deferral_candidate` with **human** `deferral_reason` (name the key and what is unavailable — **never** bare enum `mcp_export_failed` / `no_cookie` / `page_id_unresolved` alone) — **no** field obligations for that key. Prefer fixing page_id via step **3b** over deferral.
...
- **Obligation one-shot** — Skipping **3c** per-requirement subprocesses collapses invariants into COVERAGE `!` rows; parent must fan out. **`requirement_passes[]`** makes the fan-out checkable: one entry per resolved requirement, numbers recomputed by the verifier.
- **Inventory undercut** — Transcribing two rows of a nine-row table caps every downstream threshold at two. The verifier re-derives the inventory from `snippet_text`, so omissions fail; a truncated snippet fails on the `source_chars` ratio.
