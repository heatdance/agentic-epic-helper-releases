...
6. **Epic requirement analysis** (optional): use **`ANALYSE:`** + Epic key (optional **`include_closed=yes`**, optional **`benchmark_suite=`** / **`benchmark_attempt=`**) and follow [`.cursor/pipelines/analysis.md`](../pipelines/analysis.md) → **`{EpicDir}<KEY>-analysis.json`** and **`<KEY>-analysis.md`** ( **`{EpicDir}`** per [docs/benchmark-contract.md](../../docs/benchmark-contract.md)). Delete **`{EpicDir}temp/`** when done.
6b. **Epic regression test drafts** (optional): after **`-coverage.json`** exists, use **`TEST-PREP:`** + Epic key (optional **`map_only=yes`**, optional benchmark tokens) and follow [`.cursor/pipelines/test-prep.md`](../pipelines/test-prep.md) → **`{EpicDir}<KEY>-tests.json`** and **`<KEY>-tests.md`**. Delete **`{EpicDir}temp/`** when done.
6c. **Epic optional E2E materialization** (optional): after **`-tests.json`** exists and the environment is ready (app URL, Playwright MCP, optional postgres-ctqa + tunnel), use **`TEST-EXEC:`** + Epic key and follow [`.cursor/pipelines/test-exec.md`](../pipelines/test-exec.md) → **`{EpicDir}tests/*.spec.ts`** and **`<KEY>-test-exec.json`**. Delete **`{EpicDir}temp/`** when done.
