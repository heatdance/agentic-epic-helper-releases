...
- **Deliverable**: <!-- test cases per docs/tc-ref, DR per docs/dr-ref; Epic Smart Checklist via COVERAGE: + .cursor/pipelines/coverage.md; Epic analysis via ANALYSE: + .cursor/pipelines/analysis.md; Epic regression test drafts via TEST-PREP: + .cursor/pipelines/test-prep.md (after coverage); optional E2E materialization via TEST-EXEC: + .cursor/pipelines/test-exec.md (after -tests.json; env-dependent); harness drift after new pipeline/template/tool via SYNC: + .cursor/pipelines/sync.md on develop/main only -->
...
9. Produce the **requested artifact** using [docs/tc-ref](../../docs/tc-ref) or [docs/dr-ref](../../docs/dr-ref) as the structural reference, or **coverage** outputs from step 5 when the deliverable is Epic Smart Checklist coverage ([`.cursor/pipelines/coverage.md`](../pipelines/coverage.md)), or **analysis** outputs from step 6 when the deliverable is requirement analysis, or **tests** outputs from step 6b when the deliverable is regression test drafting ([`.cursor/pipelines/test-prep.md`](../pipelines/test-prep.md)), or **test-exec** outputs from step 6c when the deliverable is optional Playwright materialization ([`.cursor/pipelines/test-exec.md`](../pipelines/test-exec.md)).
