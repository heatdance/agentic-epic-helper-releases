...
- **`BENCHMARK-NEXT:`** + **`<suite_id>`** — execute **exactly one** queue step (see [`../HOW-TO.md`](../HOW-TO.md)).

**Prerequisite**: Read [`../HOW-TO.md`](../HOW-TO.md).

**Hub shadow alternative:** To keep QA durable files under **`.cursor/benchmark/runs/<suite_id>/attempt-<nn>/shadow/<KEY>/`** instead of **`epics/<KEY>/`**, follow **[`/crtqa-benchmark`](../../../commands/crtqa-benchmark.md)** and pass **`benchmark_suite=`** / **`benchmark_attempt=`** on each pipeline line per [docs/benchmark-contract.md](../../../../docs/benchmark-contract.md). **`BENCHMARK:`** / **`BENCHMARK-NEXT:`** below describe the **coverage-bench** orchestration (**`coverage-bench/runs/run-*`**) unchanged for legacy workflows.
...
**`.cursor/benchmark/coverage-bench/runs/run-<suite_id>/`**
...
2. Create **`.cursor/benchmark/coverage-bench/runs/run-<suite_id>/attempts/<EPIC>/`** for each epic (empty dirs).
...
1. Load **`.cursor/benchmark/coverage-bench/runs/run-<suite_id>/support/queue.json`** and **`support/progress.json`**.
...
3. Optionally run [`scripts/compare_runs.py`](../scripts/compare_runs.py) per epic:  
   `python .cursor/benchmark/coverage-bench/scripts/compare_runs.py --benchmark-root .cursor/benchmark/coverage-bench --epic <EPIC> --kind prep --suite-id <suite_id>`  
...
4. Write **`runs/run-<suite_id>/SUMMARY.md`** with:
   ...
   **`SUMMARY.md` is canonical inside the suite** for links and rollup. Hypotheses listed there stay **high-level**; **portable** pipeline change proposals (transferable rules mapped to playbooks/templates) belong in **`run-results/`** under the **Generalized pipeline recommendations** heading per **`run-results/README.md`**. Humans/agents additionally publish adjustment-oriented narratives under **`.cursor/benchmark/run-results/`** (markdown only — author or SDK-generate stubs following that README’s naming convention) so orchestrators can target **`.cursor/pipelines/*.md`** without mixing JSON artifacts into that folder.

   **`SUMMARY.md` must not** prescribe production **`TEST-PREP:`** as a mandatory follow-on. Test artifacts stay **operator opt-in**: **[`../../BENCHMARK_RUNBOOK.md`](../../BENCHMARK_RUNBOOK.md)** **`TEST_PREP: yes`** on PATH **A**, PATH **B** with **`TEST_PREP: yes`**, or optional PATH **T** (`test-bench/` atomic queue).
