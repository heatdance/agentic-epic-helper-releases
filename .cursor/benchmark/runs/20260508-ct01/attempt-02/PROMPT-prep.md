...
2. **Shadow only:** include **`benchmark_suite=`** and **`benchmark_attempt=`** on the **same line** as the trigger; output under **`attempt-02/shadow/CRT-639/`** ([`docs/benchmark-contract.md`](../../../../../docs/benchmark-contract.md)).
3. **One run index per folder** — use **`2`** assigned to this session (matches **`attempt-02/`**).

## Trigger (single message line)

```text
EPIC-PREP: CRT-639 benchmark_suite=20260508-ct01 benchmark_attempt=2
```

## After completion

- Confirm **`.cursor/benchmark/runs/20260508-ct01/attempt-02/shadow/CRT-639/`** has progressing prep artifacts (e.g. **`CRT-639-ref.json`** per playbook).
