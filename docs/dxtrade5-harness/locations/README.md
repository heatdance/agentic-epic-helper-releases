...
- `ctqa.json` — `https://ctqa.prosp.devexperts.com/` (retail shell bootstrap; re-run waves after login to replace drift with live `verify` data)
...
**Parity (compass vs locations):** run `python automation/tools/dxtrade5-harness/check_locations_parity.py` from repo root.
